...
// Simulated getLang function
function getLang() {
    // Replace with actual logic to determine language based on context
    return "en"; // For example, returning "en" for English
}
...
// Function to read JSON data from file with error handling
...
// Function to write JSON data to file with error handling
...
module.exports.run = async function ({ message, event, args }) {
    ...
    const dataAntiChangeInfoBox = readJSONFile(`./data/${threadID}.json`) || {};
    ...
    async function checkAndSaveData(key, data) {
        ...
        writeJSONFile(`./data/${threadID}.json`, dataAntiChangeInfoBox);
    ...
    switch (args[0]) {
        ...
        case "avatar":
        case "image": {
            const imageSrcFilePath = `./data/${threadID}_imageSrc.json`;
            if (!fs.existsSync(imageSrcFilePath))
                return message.reply(langs[getLang()]["missingAvt"]);

            const imageSrc = readJSONFile(imageSrcFilePath);
            if (!imageSrc) return;

            const newImageSrc = await uploadImgbb(imageSrc); // Replace with your image upload function
            await checkAndSaveData("avatar", newImageSrc.image.url);
            break;
        }
        case "name": {
            const threadNameFilePath = `./data/${threadID}_threadName.json`;
            const threadName = readJSONFile(threadNameFilePath);
        ...
        case "nickname": {
            const membersFilePath = `./data/${threadID}_members.json`;
            const members = readJSONFile(membersFilePath);
            if (!members) return;

            const nicknameData = members.map(user => ({ [user.userID]: user.nickname })).reduce((a, b) => ({ ...a, ...b }), {});
            await checkAndSaveData("nickname", nicknameData);
            break;
        }
        case "theme": {
            const threadThemeIDFilePath = `./data/${threadID}_threadThemeID.json`;
            const threadThemeID = readJSONFile(threadThemeIDFilePath);
            if (!threadThemeID) return;

            await checkAndSaveData("theme", threadThemeID);
            break;
        }
        case "emoji": {
            const emojiFilePath = `./data/${threadID}_emoji.json`;
            const emoji = readJSONFile(emojiFilePath);
            if (!emoji) return;
...
module.exports.handleEvent = async function ({ message, event }) {
    ...
    switch (logMessageType) {
        case "log:thread-image": {
            const dataAntiChange = readJSONFile(`./data/${threadID}_dataAntiChangeInfoBox.json`) || {};
            ...
            if (role < 1 && api.getCurrentUserID() !== author) {
                ...
                if (!imageSrc) {
                    writeJSONFile(`./data/${threadID}_dataAntiChangeInfoBox.json`, "REMOVE");
                    return;
                }

                const newImageSrc = await uploadImgbb(imageSrc); // Replace with your image upload function
                writeJSONFile(`./data/${threadID}_dataAntiChangeInfoBox.json`, newImageSrc.image.url);
            }
            break;
        }
